...
export const referencePortalSubmissionSchema = z.object({
    ...
    email: z.string().email("Invalid email for Referee 1").max(70, "Email is too long"),
    mobile: z.string().regex(/^\+?\d{8,15}$/, "Invalid phone number").max(15, "Phone Number is too long"),
    bankName: z.string().min(1, "Bank Name is required").max(80, "Bank Name is too long"),
    ...
    accountHolderName: z.string().min(1, "Account Holder Name is required").max(50, "Account HolderName is too long"),
    accountHolderEmailAddress: z.string().max(70, "Account Holder Email Address is too long").optional(),
})
